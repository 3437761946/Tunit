...
public class guis
{
	...
	public gui tool6=new gui(); // 地块层「选框」工具（图块整格移动，连带装饰层等所有图块层）
	...
	public gui codetable=new gui(); // 对象模式右侧「创建/选框」下方的代码表查看入口
	...
	public gui collabLive=new gui();   // 联机快捷面板入口（地图编辑界面，保存按钮上方留白处）

	public gui mqOK=new gui();
	public gui mqCancel=new gui();
	...
	public guis(){
		...
		codetable.setp(85,100,46,55);   // 「创建/选框」下方：代码表查看入口
		mqOK.setp(28,45,44,53);
		mqCancel.setp(51,68,44,53);
		...
		collabLive.setp(0,15,47,57);   // 竖屏：左列保存按钮上方的留白
		...
		collabLive.setl(0,10,55,66);   // 横屏：左列保存按钮上方的留白
		...
		codetable.setl(90,100,50,60);   // 「创建/选框」下方：代码表查看入口
		mqOK.setl(32,47,42,52);
		mqCancel.setl(53,68,42,52);
